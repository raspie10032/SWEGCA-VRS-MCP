...
MODULES = {"recall_context_v2.py": "recall", "session_start.py": "session_start", "precompact_snapshot.py": "precompact",
           "memory_use_log.py": "read_log", "bash_backslash_guard.py": "guard_backslash", "log_label_guard.py": "guard_label",
           "unopened_edit_guard.py": "guard_unopened", "stop_reindex_v2.py": "reindex", "usage_ledger.py": "usage",
           "repeat_ledger.py": "repeats", "hook_change_check.py": "hook_check"}
...
def check():
    ...
    for name, events in scripts.items():
        ...
        body = MODULES.get(name)                      # the shim's body lives in the package (2026-09-18)
        if body:
            body_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), body + ".py")
            if os.path.isfile(body_path):
                mtime = max(mtime, os.path.getmtime(body_path))
