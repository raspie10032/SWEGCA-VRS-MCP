...
def recall_memory(
    ...
    signal: DejaVuSignal,
    *,
    navigation_cues: Iterable[str] = (),
) -> RecallResult:
    """Recall related episodes, optionally expanding an anonymous trigger.

    Main-owned navigation may add retrieval cues after Deja vu. They are not
    current observations or authority; the caller records them separately.
    Original matched cues remain included and the trigger is never rewritten.
    """
    ...
    from .mosaic_semantic_family_directory import semantic_family_directories, family_keys, family_spans
    directories = tuple(semantic_family_directories(index))
    families = dict.fromkeys(parent for key in tuple(identifiers) for parent in family_keys(directories, key))
    dependencies = {}
    for parent in families:
        for span in family_spans(directories, parent):
            for key in span:
                identifiers.add(key)
                dependencies[(key, parent)] = None
    candidates = []
    ...
    return RecallResult(signal.query, tuple(candidates), index.snapshot_id,
                        source_dependencies=tuple(sorted(dependencies)))
...
class CurrentEvidenceVerdict:
    ...
    def __post_init__(self) -> None:
        ...
        if self.verdict in {"available", "retained"} and not self.current_evidence_refs:
            raise ValueError("experience availability requires current snapshot provenance")
        if (
...
def current_experience_verdict(
    episode: ReplayedEpisode, *, memory_snapshot_id: str,
    vrs_snapshot_id: str, current_strength: float | None = None,
    proposition: str | None = None,
) -> CurrentEvidenceVerdict:
    """Check availability/retention, not historical truth or new World authority.

    Only an actual current VRS lookup can establish retained promotion. Without
    it, the record remains available with its original verification metadata.
    A missing new observation is not a refutation or a request to re-prove it.
    """
    import math
    if current_strength is not None and (not math.isfinite(current_strength) or current_strength < 0):
        raise ValueError('invalid current VRS strength')
    _text(memory_snapshot_id, 'current memory snapshot')
    _text(vrs_snapshot_id, 'current VRS snapshot')
    retained = current_strength is not None and current_strength >= 1.0
    # Outcome/verification labels describe history, not a current logical
    # contradiction. Same-proposition opposing evidence is assessed separately.
    return CurrentEvidenceVerdict(
        episode.episode_id, proposition or 'experience:' + episode.episode_id,
        'retained' if retained else 'available',
        'current VRS promotion retained; absence of fresh evidence is not refutation' if retained else
        'record available with original uncertainty; not promoted or fresh factual support',
        ('memory-snapshot:' + memory_snapshot_id, 'vrs-snapshot:' + vrs_snapshot_id,
         *episode.source_addresses), (),
    )
...
def re_evidence_memory(
    ...
    *,
    ...
    # Availability/retention is usable experience, NOT new factual support.
    # A missing fresh observation does not contradict existing experience.
    usable_experience = any(row.verdict in {"available", "retained"} for row in judgments)
    insufficient = not conflict and not support and not refutation and not usable_experience
