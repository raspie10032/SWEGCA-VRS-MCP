...
- Add `swegca-vrs2-mcp --socket …` and `swegca-vrs-mcp --native-socket …` for an
  existing compatible native VRS2 main. The native engine is not bundled.
- Add eight native memory tools with a preferred `memory_context` workflow,
  bounded fair progress, exact source/revision/current-verdict joins, visible
  conflicts, explicit partial coverage and full source continuation.
- Preserve main identity/experience/authority, enforce request/view/snapshot
