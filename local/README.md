...
* evaluation helpers (2026-09-21 evening, all from the scratchpad of the runs): `ag_run_eval_seq.sh "agB:gemini-3.8-flash-medium …"` (runs in sequence, scores each, one line per run in `eval-seq.log`), `ag_watch_run.py <run_dir>` (emits checkpoints / writes / MCP calls / errors from the cascade db as they land), `ag_steps_since.py <run_dir> <idx>` (the steps of a run from an index), `ag_cancel_run.py <run_dir> "<reason>"` (CancelCascadeInvocation + `run.json.aborted`), `ag_mcp_swap.py eval|live|show` (points Antigravity's `swegca-vrs2` MCP entry at the evaluation or the live store, copy-first, RefreshMcpServers), `ag_restart_eval_daemon.py <state_dir>` (a daemon must be restarted after a code change — the bridge exe may stay), `ag_verify_live.py <state_dir>` (the bridge's status → context → pages → release while rows land every 0.5 s in the agent's live journal: the check that would have caught the bounded-receipt defect), `ag_gate_check.py <run_dir> <receipts_dir>` (per boundary: the last conversation line, the receipt that reached it, its lag after the checkpoint)
