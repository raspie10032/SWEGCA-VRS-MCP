...
| M9a (`route pending`) | Supersede A with B, then advance either A's or B's validity chain before trying to compensate the supersession using its original receipt | Both touched current heads must still equal that transaction's successors; a later transition on either chain refuses compensation, leaves both chains unchanged, preserves unrelated cognition, and keeps every original experience replayable | `ARCHITECTURE_SPEC.md@5901a5a` §4.8-4.9; inventory §6 failure 10. The old `mosaic_versioned_memory.py@3bddcb7:306-360` lacks this check and is source defect evidence, not the target behavior. |
...
| M11 | Call the pure stage kernel for each forward, rollback, retraction and recovery edge; then use each skipped or reversed edge, an unknown edge value and an unknown stage value | Only the source's edges return a successor; each refusal leaves the output stage unchanged; recovery has no edge from completed or rolled back | Linked transaction :114-167, :265-272, :295-312, :343-360 |
| M11a | Ask whether recovery may tolerate a failed memory rollback at each of the six stages and an unknown stage | Only `prepared` returns true; whether rollback is attempted still depends on the presence of an update ID | Linked transaction :150-159 |
| M11b (`route pending`) | Begin a transaction while another is incomplete or its ID already exists; change a stored stage after another writer advanced it | Begin refuses both cases; stage change uses the exact stored predecessor and fails without publishing on a head mismatch | Linked transaction :75-130; inventory §3G, §9 |
