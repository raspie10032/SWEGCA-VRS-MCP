...
| M9a (`route pending`) | Publish a validity update, publish a later linked successor, then try to roll back the older update using its original receipt | Current linked-head check refuses the stale rollback; neither successor validity nor unrelated later cognition changes, and every original experience remains replayable | `ARCHITECTURE_SPEC.md@5901a5a` §4.8-4.9; inventory §6 failure 10. The old `mosaic_versioned_memory.py@3bddcb7:306-360` lacks this check and is source defect evidence, not the target behavior. |
