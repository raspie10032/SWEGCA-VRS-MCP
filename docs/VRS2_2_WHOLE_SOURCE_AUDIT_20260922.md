...
| `cue_shards.py`, `exact_replay.py`, `read_projection.py`, `projected_recall.py`, `csr_cache.py` | Cue-to-address directory, original capsule, current VRS projection and cold recall. The projected path currently reads exact Replay capsules while forming Déjà vu and builds candidates before the Déjà vu receipt; this violates the requested physical stage order and leaves the 1 ms transition unproved. |
