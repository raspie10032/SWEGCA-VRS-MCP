#include "native_cue_rebuild.hpp"
...
struct CueWorker {
    ...
    std::deque<CueWork> queue;
    bool done = false;
};

// One cue always enters one worker in increasing journal sequence order.
...
// SWEGCA: user@2026-09-22:89-92
NativeCueRebuildCount rebuild_native_cue_directory(
    ...
    if (!cues.fresh())
        throw std::runtime_error("native_cue_rebuild_requires_fresh_directory");
    NativeCueRebuildCount count{};
    std::array<CueWorker, rebuild_workers> workers;
    ...
    try {
        ...
        journal.visit_addressed_rows([&](JournalRow&& stored,
            ...
            if (cancelled.load())
                throw std::runtime_error("native_cue_rebuild_cancelled");
            ...
            if (!original || !header || original->sequence > stored.sequence ||
                ...
                throw std::runtime_error("native_cue_original_address_missing");
            ...
            if (projection.pair_snapshot_id != stored.pair_id)
                throw std::runtime_error("native_cue_projection_source_changed");
            ++count.distinct_originals;
            std::array<std::vector<std::string>, rebuild_workers> groups;
            const std::set<std::string> posting_cues(
                projection.posting_cues.begin(), projection.posting_cues.end());
            count.distinct_postings += posting_cues.size();
            for (const auto& cue : posting_cues)
                groups[worker_for(cue)].push_back(cue);
            for (std::size_t number = 0; number < rebuild_workers; ++number) {
                ...
                if (cancelled.load())
                    throw std::runtime_error("native_cue_rebuild_cancelled");
                worker.queue.push_back(CueWork{
                    std::move(groups[number]), identifier, stored.sequence});
                lock.unlock();
    ...
    if (count.journal_rows != journal.row_count() ||
        count.distinct_originals + count.duplicate_observations !=
            count.journal_rows)
        throw std::runtime_error("native_cue_rebuild_count_changed");
    return count;
}
