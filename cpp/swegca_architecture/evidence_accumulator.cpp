...
// A single-element node for `like`'s container type, charged to the same
// ledger, built in the prepare phase so the commit phase only splices it.
...
static Digest256 bind_admitted(const MemoryLedger::Account& memory, const ClaimRevision& claim,
...
// SWEGCA: src/swegca/mosaic_bounded_world_write.py@5901a5a:153-172
Digest256 evidence_binding_digest(const MemoryLedger::Account& memory, std::string_view claim,
...
EvidenceAccumulator::EvidenceAccumulator(const MemoryLedger::Account& memory, ClaimRevision claim,
...
AdmissionResult EvidenceAccumulator::admit(const EvidenceObservation& observation,
    ...
    const auto source = identity_id(source_ids_, source_text);
    ...
    auto& axis = axes_[observation.axis];
    const GroupKey group_key{source, observation.context};
    ...
    TextIds::node_type source_id_node;
    if (!source_ids_.contains(source_text))
        source_id_node = detached(source_ids_, Text(source_text, alloc), source);
    ...
    if (!sources_.contains(source)) source_node = detached(sources_, source);
    Set<Digest256>::node_type context_node;
    if (!contexts_.contains(observation.context))
        context_node = detached(contexts_, observation.context);
    ...
    splice(source_ids_, source_id_node);
    ...
    splice(sources_, source_node);
    splice(contexts_, context_node);
    ...
    tally_.axis_support[observation.axis] = rounded_sum(support_sum);
    tally_.axis_refute[observation.axis] = rounded_sum(refute_sum);
    tally_.axis_source_diversity[observation.axis] =
        static_cast<std::uint32_t>(std::min(axis.sources.size(), axis.producers.size()));
...
EvidenceDecision::EvidenceDecision(const MemoryLedger::Account& memory, ClaimRevision claim,
